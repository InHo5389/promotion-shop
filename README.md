...
- MAU 2,000만, DAU 500만 사용자 대상 서비스 설계
...
**[도입 배경]**
> 시스템 설계 목표: MAU 2,000만, DAU 500만 사용자를 대상으로 한 쿠폰 서비스 구축 가정 <br/>
>> QPD(Query Per Day) = DAU × 발행 액션 수 × 세션당 쿼리 수 = 500만 × 2 × 3 = 3,000만건 <br/>
>> 읽기 QPS = 2,000만건 ÷ 86,400초 ≈ 231 QPS (쿠폰 정책 조회 + 발행 수량 체크) <br/>
>> 쓰기 TPS = 1,000만건 ÷ 86,400초 ≈ 116 TPS (쿠폰 저장) <br/>
>> Peak 시간대에는 평상시의 4~5배인 초당 500+ TPS의 쿠폰 발행 API 처리 가정 <br/>
>> 참고로 피크 쓰기 TPS는 116 × 4~5배 ≈ 463~579 TPS라서 "500+ TPS"로 표기했습니다. 기존과 같은 읽기:쓰기 = 2:1 비율을 유지했습니다. <br/>
